"""CHUNKED architecture eval: foveation binder -> ONE object-agnostic CHUNKED goal-conditioned motor
(predict K-step action sequence, execute R, re-plan). Chunking restores pi0.5's smoothness + compounding
robustness that single-step distillation threw away. Measures binding, grasp, FULL pick-place success.

Run: PYTHONPATH=third_party/libero MUJOCO_GL=egl .venv/bin/python motor_distill/eval_e2e_chunked.py --n 12 --seed 7
"""
from __future__ import annotations
import argparse, glob, pathlib, pickle, re
import jax, jax.numpy as jnp, numpy as np
from cf_harness import parse_bddl, resolve_bodies, body_pos
from distill_chunked import head_apply_chunked


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--bddl-dir", default="data/libero_pro/bddl_files/libero_object_task")
    ap.add_argument("--head", default="runs/chunked_head.pkl"); ap.add_argument("--container", default="basket")
    ...
    ch = jax.tree.map(jnp.asarray, pickle.load(open(args.head, "rb")))
    ...
    chunk_fn = jax.jit(head_apply_chunked)
    def predict_chunk(goalT, goalC, obs):
        ee = np.asarray(obs["robot0_eef_pos"], np.float32)
        return np.asarray(chunk_fn(ch, jnp.asarray(ee-goalT), jnp.asarray(ee-goalC),
                                   jnp.asarray(np.asarray(obs["robot0_eef_quat"],np.float32)),
                                   jnp.asarray(np.asarray(obs["robot0_gripper_qpos"],np.float32))))  # (K,7)
    ...
    print("EVAL_E2E_CHUNKED_EXIT=0", flush=True)
