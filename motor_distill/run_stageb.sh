#!/bin/bash
# Stage B: head closed-loop on perturbed scenes vs raw Pi0.5. Parallel over tasks x perts.
#   nohup env V=plain N=40 bash motor_distill/run_stageb.sh > /root/stageb.log 2>&1 &
set -u
cd "$(dirname "$0")/.."
...
V=${V:-plain}; N=${N:-40}; TASKS=${TASKS:-3 9}; PERTS=${PERTS:-5 10}
mkdir -p logs/stageb
for T in $TASKS; do for P in $PERTS; do
  ( echo N | uv run --no-sync python motor_distill/eval_stageb.py \
      --ckpt data/keystone/ckpt/head_${V}.pt --task-idx "$T" \
      --pert-dir data/keystone/pert${P} --ref-dir data/keystone/pert0 \
      --n "$N" --replan 2 --g-mode nearest \
      > "logs/stageb/sb_${V}_t${T}_p${P}.log" 2>&1 ) &
done; done
wait
echo "STAGEB_DONE"
