...
def main():
    ...
    def predict(active, obs):
        ee = np.asarray(obs["robot0_eef_pos"], np.float32)
        return np.asarray(motor_fn(mh, jnp.asarray(ee-active), jnp.asarray(np.asarray(obs["robot0_eef_quat"],np.float32)),
    ...
    for bf in bddls:
        ...
        for step in range(args.horizon):
            ...
            recent.append((step, predict(active, obs))); recent = recent[-K:]
            preds, ages = [], []
            for (s, ck) in recent:
                j = step - s
            ...
            if (not held) and close_cnt > 8 and lifted > 0.02: held = True   # relay latch: grasped -> carry
        ...
        print(f"  {nm(T):14s} bind={'Y' if bind_ok[-1] else '.'} grasp={'Y' if lifted>0.04 else '.'}({lifted*100:3.0f}cm) "
              f"held={int(held)} obj2basket={objdist*100:4.0f}cm placed={'Y' if placed else '.'}", flush=True)
    n = len(bind_ok)
    print(f"\n=== LEARNED MOTOR v2 (relay+L1/BCE+chunk+ensemble) + foveation binder (N={n}, seed={args.seed}) ===", flush=True)
    print(f"  obj binding: {np.mean(bind_ok):.2f}   grasp: {np.mean(grasp_ok):.2f}   FULL: {np.mean(full_ok):.2f}", flush=True)
