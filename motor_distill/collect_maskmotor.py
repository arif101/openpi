"""STAGE 1-DART (recovery data) for the GOAL-CORRECTING motor. Roll out frozen pi0.5 on standard libero_object (it succeeds
...
def target_mask(sim, body, seg_native, depth_norm, R, cam="agentview"):
    """Binary mask (128) of the target object in the agentview, from instance-seg at the object's projected center."""
    import robosuite.utils.camera_utils as cu
    from openpi_client import image_tools
    w2p = cu.get_camera_transform_matrix(sim, cam, R, R); pos = body_pos(sim, body)
...
def main():
    ...
    p.add_argument("--dart-noise", type=float, default=0.02)   # DART: action-noise std (m) to perturb state -> pi0.5 corrects
    p.add_argument("--dart-prob", type=float, default=0.35)   # prob a step is perturbed
    args = p.parse_args()
    import torch
    ...
    for bf in bddls:
        ...
        for seed in seeds:
            ...
            for t in range(nt):
                env = OffScreenRenderEnv(bddl_file_name=bf, camera_heights=256, camera_widths=256, camera_depths=True, camera_segmentations="instance")
                ...
                for step in range(args.horizon):
                    ...
                    if chunk is None or ci >= args.replan:
                        ...
                        ag = image_tools.resize_with_pad(np.ascontiguousarray(np.asarray(obs["agentview_image"])[::-1,::-1]), 128, 128)
                        mk = target_mask(sim, cb if held else rb[T], np.asarray(obs["agentview_segmentation_instance"]), np.asarray(obs["agentview_depth"]), 256)
                        WR.append(image_tools.convert_to_uint8(wr)); GR.append(goal_rel.copy())
                        PR.append(prop); HE.append(int(held)); CH.append(chunk.copy())
                        AG.append(image_tools.convert_to_uint8(ag)); MK.append(mk.astype(np.uint8))
                    ...
                    # DART: perturb the executed action -> robot drifts off-trajectory; pi0.5 re-plans the CORRECTION next chunk
                    if args.dart_noise > 0 and (np.random.rand() < args.dart_prob) and not held:
                        a[:3] = a[:3] + np.random.normal(0, args.dart_noise, 3).astype(np.float32)
                        ci = args.replan   # force re-plan next step from the perturbed state
                ...
                if ok and len(WR) > 3:
                    ...
                    np.savez_compressed(out / f"{stem[:30]}_s{seed}_t{t}.npz",
                                        ...
                                        chunk=np.asarray(CH, np.float32), agview=np.asarray(AG, np.uint8), tmask=np.asarray(MK, np.uint8))
