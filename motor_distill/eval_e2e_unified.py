"""UNIFIED architecture eval: foveation binder -> ONE object-agnostic goal-conditioned motor (no primitives,
no gate). The head gets both goals relative to the EE + grip and drives the whole pick->place as one smooth
policy. Measures binding, grasp, FULL pick-place success.

Run: PYTHONPATH=third_party/libero MUJOCO_GL=egl .venv/bin/python motor_distill/eval_e2e_unified.py --n 12 --seed 7
"""
...
def main():
    ...
    ap.add_argument("--head", default="runs/unified_head.pkl"); ap.add_argument("--container", default="basket")
    ...
    def act(goalT, goalC, obs):
        ee = np.asarray(obs["robot0_eef_pos"], np.float32)
        return np.array(head_apply_unified(uh, jnp.asarray(ee-goalT), jnp.asarray(ee-goalC),
    ...
    for bf in bddls:
        ...
        if goalT is None or goalC is None:
            grasp_ok.append(0); full_ok.append(0); env.close()
            print(f"  {nm(T):14s} NO BIND", flush=True); continue
        z0 = body_pos(sim, rb[T])[2]; lifted = 0.0
        for step in range(args.horizon):                      # ONE policy drives the whole task
            obs, _, done, _ = env.step(act(goalT, goalC, obs).tolist())
    ...
    n = len(bind_ok)
    print(f"\n=== UNIFIED motor + foveation binder (N={n}, seed={args.seed}) ===", flush=True)
    ...
    print("EVAL_E2E_UNIFIED_EXIT=0", flush=True)
