...
def init_unified(key, din=12, dh=256, dout=7):
    ...
    return {"w1": s(k[0], din, dh), "b1": jnp.zeros(dh),
            ...
            "k_attract": jnp.array(2.0)}


def head_apply_unified(p, ee_rel_obj, ee_rel_cont, quat, grip):
...
def main():
    ...
    for step in range(args.steps):
        ...
        if step % 500 == 0 or step == args.steps - 1:
            ...
            print(f"step {step:5d} mse={float(Lv):.4f} dir_cos={dc:+.3f} grip_acc={ga:.3f} k={float(params['k_attract']):.2f}", flush=True)
